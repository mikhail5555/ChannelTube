...
YOUTUBE_ID_FROM_FILENAME_REGEX = re.compile(r"(?<=\[)[a-zA-Z0-9\-_]{11}(?=])")
...
class DataHandler:
    def __init__(self):
        ...
        self.subtitle_languages = os.environ.get("subtitle_languages", "en,nl").split(",")
    ...
    def get_list_of_videos_from_youtube(self, channel, existing_youtube_ids):
    ...
    def get_list_of_youtube_ids_from_channel_folder(self, channel_folder_path: str) -> Set[str]:
        youtube_ids = set()
    ...
    def process_channel(self, channel):
        try:
            ...
            existing_youtube_ids = self.get_list_of_youtube_ids_from_channel_folder(channel_folder_path)

            self.general_logger.warning(f'Getting list of videos for channel: {channel["Name"]} from {channel["Link"]}')
            item_download_list = self.get_list_of_videos_from_youtube(channel, existing_youtube_ids)
